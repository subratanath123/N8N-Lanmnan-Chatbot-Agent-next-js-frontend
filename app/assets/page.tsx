...
export default function AssetsPage() {
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    try {
      // Build URL with optional folderPath
      const uploadUrl = new URL(`${backendUrl}/v1/api/assets/upload`);
      if (currentFolder) {
        uploadUrl.searchParams.append("folderPath", currentFolder);
      }

      console.log("Uploading files to:", uploadUrl.toString(), "currentFolder:", currentFolder);
  ...
  const STORAGE_KEY = `assets_folders_${currentFolder || 'root'}`;

  const saveFoldersToStorage = useCallback((folderList: Folder[]) => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(folderList));
      console.log("Saved folders to localStorage:", STORAGE_KEY, folderList);
    } catch (err) {
      console.error("Failed to save to localStorage:", err);
    }
  }, [STORAGE_KEY]);

  const loadFoldersFromStorage = useCallback((): Folder[] => {
    try {
      const stored = localStorage.getItem(STORAGE_KEY);
      if (stored) {
        const folderList = JSON.parse(stored);
        console.log("Loaded folders from localStorage:", STORAGE_KEY, folderList);
        return folderList;
      }
    } catch (err) {
  ...
  const fetchFolders = useCallback(async () => {
    if (!backendUrl) {
      ...
      const storedFolders = loadFoldersFromStorage();
      setFolders(storedFolders);
      return;
    }
    try {
      const headers = await getHeaders();
      console.log("Fetching folders with headers:", headers);
      
      const url = new URL(`${backendUrl}/v1/api/assets/folders`);
      if (currentFolder) {
        url.searchParams.append("parentFolder", currentFolder);
      }
      
      console.log("Fetching from URL:", url.toString());
      
      const res = await fetch(url.toString(), { headers });
      console.log("Fetch folders response:", { status: res.status, ok: res.ok });
      
      if (res.ok) {
        const folderList: string[] = await res.json();
        console.log("Fetched folders from backend:", folderList);
        
        // Merge with localStorage folders
        const storedFolders = loadFoldersFromStorage();
        const backendFolders = folderList.map(name => ({ name, createdAt: new Date().toISOString() }));
        
        // Combine and deduplicate by name
        const combined = Array.from(
          new Map([
            ...storedFolders.map(f => [f.name, f]),
            ...backendFolders.map(f => [f.name, f])
          ].map(([name, folder]) => [name, folder])).values()
        );
        
        console.log("Combined folders (backend + localStorage):", combined);
        setFolders(combined);
      } else {
        // Use localStorage as fallback
        const storedFolders = loadFoldersFromStorage();
        console.log("Using localStorage folders as fallback:", storedFolders);
        setFolders(storedFolders);
        
        const errorText = await res.text();
        console.error("Failed to fetch folders:", { status: res.status, error: errorText });
      }
    } catch (err) {
  ...
  }, [backendUrl, currentFolder, getHeaders, loadFoldersFromStorage]);

  /* ── sync folders to storage whenever they change ── */
  useEffect(() => {
    saveFoldersToStorage(folders);
  }, [folders, saveFoldersToStorage]);

  /* ── load folders on mount and when folder changes ── */
  useEffect(() => {
    fetchFolders();
    
    // If folders are empty, try to load from localStorage immediately
    setTimeout(() => {
      if (folders.length === 0) {
        const storedFolders = loadFoldersFromStorage();
        if (storedFolders.length > 0) {
          console.log("No backend folders, loading from localStorage");
          setFolders(storedFolders);
        }
      }
    }, 500);
  }, [fetchFolders, folders.length, loadFoldersFromStorage, currentFolder]);

  /* ── create folder ── */
  const createFolderBackend = useCallback(async (folderName: string) => {
    if (!backendUrl) {
      console.error("Backend URL not configured");
      return false;
    }
    try {
      ...
      const res = await fetch(`${backendUrl}/v1/api/assets/folders`, {
        method: "POST",
  ...
  }, [backendUrl, currentFolder, getHeaders, fetchFolders]);

  /* ── delete folder ── */
  const deleteFolderBackend = useCallback(async (folderName: string) => {
    if (!backendUrl) return false;
    if (!confirm(`Delete folder "${folderName}" and all assets inside it?`)) return false;
  ...
  }, [backendUrl, currentFolder, getHeaders, fetchFolders, fetchAssets]);
  ...
  }, [fetchFolders, folders.length, loadFoldersFromStorage]);

  /* ── filtered list ── */
  const filtered = assets.filter((a) => {
    // Filter by folder - check if asset's folderPath matches current folder
    const assetFolder = a.folderPath || "";
    if (currentFolder && assetFolder !== currentFolder) return false;
    if (!currentFolder && assetFolder !== "") return false;
    ...
    <div style={{ display: 'flex', width: '100%', minHeight: '100vh', background: '#f8f9fa' }}>
      ...
      <main style={{ flex: 1, marginLeft: '280px', paddingTop: '60px', background: '#f8f9fa', minHeight: '100vh' }}>
        <PageHeader
          ...
          title="Assets"
          subtitle="Upload, organize, and manage images and videos. Store them in Supabase for easy access."
          icon={
              ...
              <MDBBtn 
                ...
                onClick={async () => {
                  ...
                  if (folderName && folderName.trim()) {
                    ...
                    const success = await createFolderBackend(folderName.trim());
                    if (!success) {
        ...
        <div className="ap">
          ...
          {currentFolder && (
            <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', color: '#64748b' }}>
              <button 
              ...
              <span>/</span>
              <span style={{ color: '#0f172a', fontWeight: 500 }}>{currentFolder}</span>
            </div>
          )}
          ...
          {!search && folders.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9ca3af', marginBottom: '10px' }}>
                FOLDERS
              </div>
                ...
                {folders.map((folder) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginBottom: '4px' }}>{folder.name}</div>
                    </div>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        deleteFolderBackend(folder.name);
                      }}
                      style={{
                        position: 'absolute',
                        top: '4px',
          ...
          >
            ...
            <span className="ap-dz-hint">or click / use the button above · files are stored securely in cloud storage</span>
          </div>
          ...
          {loadError && (
            <div className="ap-err-banner">
              ...
              <br /><small>Make sure the backend implements <code>GET /v1/api/assets</code>.</small>
            </div>
          )}
          ...
          <style jsx>{`
            .ap { max-width: 1100px; margin: 0 auto; padding: 24px 36px 60px; }
